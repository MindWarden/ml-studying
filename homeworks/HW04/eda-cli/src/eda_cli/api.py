...
@app.post(
    ...
    tags=["quality"],
    summary="Возвращает только флаги качества (включая новые эвристики из HW03)",
    description="Эндпоинт для задания 2.3.2. Демонстрирует доработки HW03.",
)
async def quality_flags_from_csv(file: UploadFile = File(...)):
    """
    Принимает CSV → использует summarize_dataset + missing_table + compute_quality_flags
    → возвращает ВСЕ флаги качества, включая твои новые (has_constant_columns, has_high_cardinality_categoricals и т.д.)
    """
    ...
    summary = summarize_dataset(df)
    missing_df = missing_table(df)
    flags = compute_quality_flags(summary, missing_df)  # ← здесь все твои новые эвристики!

    print(f"[quality-flags-from-csv] {file.filename!r} → {df.shape} → score={flags.get('quality_score')}")
