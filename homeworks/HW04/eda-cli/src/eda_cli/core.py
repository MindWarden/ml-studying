...
def compute_quality_flags(summary: DatasetSummary, missing_df: pd.DataFrame) -> Dict[str, Any]:
    ...
    constant_columns = [
        ...
        if col.unique <= 1 and col.non_null > 0  # все значения одинаковые или колонка пустая, но не NaN
    ]
    flags["has_constant_columns"] = len(constant_columns) > 0
    flags["constant_columns_count"] = len(constant_columns)
    flags["constant_columns"] = constant_columns  # может пригодиться

    # НОВАЯ ЭВРИСТИКА 2: высокая кардинальность в категориальных 
    # Считаем категориальными: не числовые и не слишком мало уникальных
    high_card_cols = []
    for col in summary.columns:
        if not col.is_numeric:  # объект, строка, категория и т.д.
            # Порог: больше 50% уникальных значений от числа строк → почти как ID
            if col.unique > n_rows * 0.5:
                high_card_cols.append(col.name)

    flags["has_high_cardinality_categoricals"] = len(high_card_cols) > 0
    flags["high_cardinality_columns_count"] = len(high_card_cols)
    flags["high_cardinality_columns"] = high_card_cols
    flags["high_cardinality_threshold_share"] = 0.5  # поясняем порог на всякий

    # Обновлённый quality_score (учитывает новые проблемы)
    score = 1.0
    score -= max_missing_share                    # чем больше пропусков → хуже
    if n_rows < 100:
        score -= 0.2
    if n_cols > 100:
        score -= 0.1
    if flags["has_constant_columns"]:
        score -= 0.15                              # константные колонки — серьёзно портят
    if flags["has_high_cardinality_categoricals"]:
        score -= 0.2                               # высокая кардинальность = сложность
