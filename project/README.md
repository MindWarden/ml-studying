...
**Краткое описание:**
Проект решает задачу бинарной классификации: для клиента телеком-оператора по его профилю (тип контракта, услуги, платежи и т.д.) предсказывается вероятность ухода. Используется открытый датасет Telco Customer Churn (IBM, 7043 клиента). Сравниваются три модели (LogisticRegression, RandomForest, GradientBoosting), лучшая по ROC-AUC обёрнута в FastAPI-сервис с эндпоинтами `/predict` и `/health`, упакована в Docker.
...
Сервис поднимется на `http://localhost:8000`. В образе уже лежит обученная модель `artifacts/model.pkl`; если её нет, обучение произойдёт автоматически на этапе `docker build`.
...
# 1) обучение модели (создаст artifacts/model.pkl)
...
# {"churn_probability":0.119,"churn_class":"No","threshold":0.5}
```
...
4. Два запроса к `/predict` для контраста:
   - «лояльный» клиент (Two year контракт, большой tenure) → вероятность ухода ~12%;
   - «рисковый» клиент (Month-to-month, tenure=1, Electronic check) → вероятность ухода ~80%.
5. Открытие `notebooks/02_models_comparison.ipynb`: сравнительная таблица метрик и ROC-кривые.
6. `report.md`: раздел «Выбор финальной модели» с обоснованием (ROC-AUC, recall, интерпретируемость).
...
- threshold классификации жёстко задан 0.5 (меняется через переменную окружения `CHURN_THRESHOLD`, но не подбирается автоматически по бизнес-метрике);
- нет аутентификации сервиса (по требованиям курса не нужна, но в проде потребуется).

**Возможные улучшения:**
- добавить тюнинг гиперпараметров через `GridSearchCV` / `Optuna`;
- подобрать threshold под максимизацию F-beta (β>1, чтобы приоритизировать recall);
- эксперименты с CatBoost/XGBoost и SHAP-интерпретацией;
- интеграция MLflow для трекинга экспериментов;
- кэширование предсказаний и метрики Prometheus в сервисе.
